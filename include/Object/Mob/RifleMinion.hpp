...
public:
    RifleMinion(sf::Vector2f position, State* state) : Enemy("RifleMinion", position)
    {
        ...
        sightRange = 600.f;
        movingAnimation = std::make_unique<HighRangeMob_MovingAnimation>(&ResourceManager::getInstance().get<sf::Texture>(Textures::ID::RifleMinion), sf::Vector2u(8, 3), 0.1f, this->position, 0.4f, this);
