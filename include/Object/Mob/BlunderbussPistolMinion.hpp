    ...
    BlunderbussPistolMinion(sf::Vector2f position, State* state) : Enemy("BlunderbussPistolMinion", position)
    {
        ...
        inventory->addWeapon(
            std::make_shared<Weapon2>(
                "BlunderbussPistolMinionSword",
                this->position, // Position of the weapon
                1.0f, // cooldowntime
                sf::Vector2(0.5f, -0.2f), // The Scale position of the bullet spawner
                std::make_unique<RangedWeaponBehavior>(state,500,20), // Ranged weapon behavior with speed
                std::make_unique<SwordAnimation>(
                    0.2f, // Total time for the animation
                    -0.3f, // Scale of the animation
                    &ResourceManager::getInstance().get<sf::Texture>(Textures::ID::BlunderbussPistolMinionBlunderbussPistol), // Texture for the gun animation
