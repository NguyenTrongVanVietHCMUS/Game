...
public:
    ...
    {
        sightRange = 250.f;
        movingAnimation = std::make_unique<HighRangeMob_MovingAnimation>(&ResourceManager::getInstance().get<sf::Texture>(Textures::ID::GoblinExecutor), sf::Vector2u(8, 3), 0.1f, this->position, 2.2, this);
        movingAnimation->speed = 175.0f;
        aiEnemy = std::make_unique<AIHighRangeEnemy>();
        inventory->addWeapon(
            std::make_shared<Weapon2>(
                ...
                1.0f, // cooldowntime
                sf::Vector2(0.5f, -0.2f), // The Scale position of the bullet spawner
                std::make_unique<RangedWeaponBehavior>(state), // Ranged weapon behavior with speed
